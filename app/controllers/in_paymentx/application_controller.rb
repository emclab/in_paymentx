module InPaymentx
  class ApplicationController < ::ApplicationController
    ...
    before_filter :load_current_token
    ...
    def return_customers_by_access_right     
      access_rights, model_ar_r, has_record_access = access_right_finder('index', InPaymentx.customer_class.to_s.underscore.pluralize, session[:user_role_ids], session[:fort_token], nil,nil,nil,nil,session[:user_id])
    ...
    def return_projects_by_access_right     
      access_rights, model_ar_r, has_record_access = access_right_finder('index', InPaymentx.project_class.to_s.underscore.pluralize, session[:user_role_ids], session[:fort_token], nil,nil,nil,nil,session[:user_id])
      return [] if access_rights.blank?
      return model_ar_r #instance_eval(access_rights.sql_code) #.present?
    end
  end
